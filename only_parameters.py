...
def init_camera():
    """Initialize camera with cv2.VideoCapture"""
...
if st.session_state.parameters_page:
    ...
    quality_info = {
        "Regular": "~32 images - Quick calibration",
        "Medium": "~50 images - Balanced quality",
        "High": "~128 images - Maximum accuracy"
    }
...
# ============================================================
    ...
    # Initialize camera
    if not init_camera():
        st.error("❌ Failed to open camera. Please check if camera is available.")
    ...
    with col_info:
        st.markdown(
            ...
            <h4>📌 How it works:</h4>
            <ul>
                <li><b style='color:#00ff00'>Green Box:</b> Pattern detected</li>
                <li><b style='color:#ff0000'>Red Box:</b> No pattern</li>
                <li><b style='color:#ffa500'>Orange Box:</b> Stabilizing 1.5s</li>
    ...
    while st.session_state.camera is not None and st.session_state.camera.isOpened():
        ...
        if not ret:
            status.error("❌ Failed to read frame from camera")
            break
